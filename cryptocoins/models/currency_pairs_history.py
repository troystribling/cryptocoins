...
class CurrencyPairsHistory(BaseModel):
    ...
    @classmethod
    def pairs(cls, timestamp_epoc, limit=None):
        if limit is None:
            ...
            return cls.raw("SELECT * FROM currency_pairs_history WHERE timestamp_epoc=%s"
                           " ORDER BY from_symbol, volume_from_24_hour DESC LIMIT %s", timestamp_epoc, limit)
    ...
    @classmethod
    def pairs_data_frame(cls, timestamp_epoc, limit=None):
        records = [record for record in cls.pairs(timestamp_epoc, limit).dicts()]
